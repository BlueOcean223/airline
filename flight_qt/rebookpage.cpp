...
void rebookPage::rebookTicket(const Flight& selectedFlight) {
    ...
    // 创建或获取数据库连接
    QSqlDatabase db;
    if (QSqlDatabase::contains("rebook_connection")) {
        db = QSqlDatabase::database("rebook_connection");
    } else {
        db = QSqlDatabase::addDatabase("QODBC", "rebook_connection");
        db.setDatabaseName(DSN);
        db.setUserName(DB_USER);
        db.setPassword(DB_PASSWORD);
    }
...
QIcon rebookPage::selectImg(const QString& airline){
    QIcon icon;
